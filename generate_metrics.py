...
def load_safetensors_into_model(model, ckpt_path):
    """Load a .safetensors checkpoint into a torch nn.Module."""
...
def load_pt_into_model(model, ckpt_path):
    """Load a .pt checkpoint into a torch nn.Module, handling state_dict wrappers."""
...
def main(args):
    ...
    if args.oasis_ckpt.endswith(".pt"):
        load_pt_into_model(model, args.oasis_ckpt)
    elif args.oasis_ckpt.endswith(".safetensors"):
        load_safetensors_into_model(model, args.oasis_ckpt)
    else:
        raise ValueError(f"Unsupported DiT checkpoint extension for {args.oasis_ckpt}")
    model = model.to(device).eval()

    # load VAE checkpoint (ViT VAE, vit-l-20.pt)
    vae = VAE_models["vit-l-20-shallow-encoder"]()
    print(f"loading ViT-VAE-L/20 from vae-ckpt={os.path.abspath(args.vae_ckpt)}...")
    if args.vae_ckpt.endswith(".pt"):
        load_pt_into_model(vae, args.vae_ckpt)
    elif args.vae_ckpt.endswith(".safetensors"):
        load_safetensors_into_model(vae, args.vae_ckpt)
    else:
        raise ValueError(f"Unsupported VAE checkpoint extension for {args.vae_ckpt}")
    ...
    for i in tqdm(range(n_prompt_frames, total_frames)):
        ...
        for noise_idx in reversed(range(1, ddim_noise_steps + 1)):
            ...
            x_noise = (
                (1 / alphas_cumprod[t]).sqrt() * x_curr - x_start
    ...
    if LPIPS_AVAILABLE and args.compute_metrics:
        ...
        try:
            ...
            temporal_dists = []
            for t_idx in range(frames_lpips.shape[0] - 1):
                frame_t = frames_lpips[t_idx: t_idx + 1]      # (1, C, H, W)
                frame_t1 = frames_lpips[t_idx + 1: t_idx + 2]  # (1, C, H, W)

                with torch.no_grad():
                    dist = lpips_model(frame_t, frame_t1)
                temporal_dists.append(dist.item())

            t_lpips = float(np.mean(temporal_dists))
